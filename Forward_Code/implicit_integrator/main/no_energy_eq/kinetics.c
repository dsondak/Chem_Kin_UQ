...
int main()
{
  ...
  if (detailed) { 
     n_species = n_species_d;
  }
  const unsigned int n_inert          = 1; // e.g. N2
  unsigned int n_inad                 = 0; // virtual species
  if (inad) {
     n_inad = 2;
  }
  const unsigned int n_atoms          = 2; // types of atoms in the system
  unsigned int n_reactions            = 5; // number of reactions
  if (detailed) {
     n_reactions = 21;
  }
  else if (inad) {
  ...
  const unsigned int n_times          = NOUT + 1;
  ...
  truth_data detailed_data("detailed_data.h5", 1, 1, n_times, n_eq_d);
  ...
  if (write_to_h5) {
     create_file(data_fname, NOUT + 1, n_eq + 1, 1); // for writing out data
  }
  ...
  reaction_info rxnMain(&chem_mixture, &reaction_set, &thermo, &nasa_mixture, &kinetics, Amat, bvec, 
                        Tdata, n_species, n_inert, n_inad, n_atoms, n_eq, n_reactions, Q);
  ...
  std::vector<double> solutionf((n_eq + 1) * (NOUT + 1) + 2, 0.0);
  ...
  for (unsigned int iout = 1; iout <= NOUT; iout++) {
    ...
    try {
       ...
       if (write_to_h5) {
          write_file(timef, solutionf, data_fname, NOUT + 1, n_eq + 1, 0, 1.0, 5.0e+06);
       }
       else {
          std::ofstream txt_solution;
          txt_solution.open("inad_solution_cvode.txt");
          for (unsigned int i = 0; i <= NOUT; i++) {
              txt_solution << timef[i] << "   ";
              for (unsigned int j = 0; j < n_eq; j++) {
                  txt_solution << solutionf[i * (n_eq + 1) + j] << "   ";
              }
              txt_solution << Tdata[timef[i]];
              txt_solution << "\n";
          }
          txt_solution.close();
       }
    ...
    for (unsigned int j = 0; j < n_eq; j++) {
        solutionf[iout * (n_eq + 1) + j] = Ith(y,j+1);
    }
    solutionf[iout * (n_eq + 1) + n_eq] = Tdata[tout];

  }

  /* Ignition data */
  solutionf[(n_eq + 1) * (NOUT + 1)]     = time_ig;
  solutionf[(n_eq + 1) * (NOUT + 1) + 1] = T_ig;
