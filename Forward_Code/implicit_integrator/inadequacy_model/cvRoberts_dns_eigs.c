...
int main()
{
  ...
  //flag = CVodeSetMaxOrd(cvode_mem, 3);
  ...
  std::cout << n_species_tot << " species kinetics problem." << std::endl;
  ...
  inad_constraints[0] = 100.0;
  inad_constraints[1] = 100.0;
  ...
  std::cout << inad_constraints << std::endl;
...
static int f(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
  ...
  std::vector<double> mole_sources_1(n_species_tot, 0.0);
  std::vector<double> mole_sources(n_species_tot, 0.0);

  // Get RHS and it's derivatives
  ...
  for (unsigned int k = 0; k < n_species_tot; k++) {
      printf("%20.16f\n %19.16f \n\n", mole_sources[k], mole_sources_1[k]);
  }
